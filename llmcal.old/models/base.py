...
class LabelsDecoder(nn.Module):
    ...
    def _decoder_only_forward(self, encoder_output, decoder_input):
        ...
        if isinstance(encoder_output["past_key_values"], tuple):
            encoder_output["past_key_values"] = DynamicCache.from_legacy_cache(encoder_output["past_key_values"][0])
...
class BaseLMClassifier(nn.Module):
