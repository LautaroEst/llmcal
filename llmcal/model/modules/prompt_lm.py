    ...
    def forward(
        ...
        for input_ids, attention_mask in zip(prompt_ids, prompt_mask):
            ...
            outputs["last_hidden_state"].append(
                torch.cat([torch.zeros(1, len(attention_mask == 0), self.config.n_embd, dtype=output["last_hidden_state"].dtype, device=output["last_hidden_state"].device), output["last_hidden_state"]])
            )
            outputs["logits"].append(
                torch.cat([torch.zeros(1, len(attention_mask == 0), self.config.vocab_size, dtype=output["logits"].dtype, device=output["logits"].device), output["logits"]])
            )
        outputs["last_hidden_state"] = torch.stack(outputs["last_hidden_state"], dim=0)
        outputs["logits"] = torch.stack(outputs["logits"], dim=0)
        return outputs

                
class LitGPTPromptClassifier(LitGPT):

    def __init__(self, model_name_or_path: str, embedding_pooling: Literal["mean", "max", "last"]):
