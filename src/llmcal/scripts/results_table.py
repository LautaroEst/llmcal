...
def create_table(results_dir, methods, metrics, methods_config, datasets, sizes):
    ...
    data_all.index = data.index.map(lambda x: ({sizes[0]: "\\rotatebox[origin=c]{90}{smallest}", sizes[-1]: "\\rotatebox[origin=c]{90}{largest}"}[x[0]], method2group(x[2]), methods_config[x[2]]["label"] ))
    ...
    noa_data = pd.DataFrame({
        ...
        "size": [""] * len(datasets),
        "result": [""] * len(datasets),
    }).pivot(index=["size","method"], columns="dataset", values="result").loc[:,datasets]
    ...
    noa_data.columns = noa_data.columns.map(lambda x: DATASETS[x]["name"])
...
def main(
    ...
    table_str = table.to_latex(escape=False, column_format="ll" + "|c" * len(datasets))
